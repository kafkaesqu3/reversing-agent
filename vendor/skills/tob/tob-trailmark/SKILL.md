...
- `functions[].id`/`symbol`/`name` and `address`/`rva` are all valid reference keys; `calls[]`
  ...
  working from.
- **`confidence` is not decorative.** Set `"certain"` only for a direct, statically-resolved call
  you can see at a fixed address in disassembly or decompilation. Set `"inferred"` (the default
  if you omit the field) for anything from `list_xrefs`/`gen_callgraph` you have not individually
...
engine._store._graph.entrypoints["<entry-function-id>"] = EntrypointTag(
