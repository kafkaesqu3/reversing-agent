...
| 9 | **The gate validates the grant the config and catalog imply, never the bytes of the generated file.** A2 and A3 run over `Get-AgentToolGrant`'s freshly recomputed list, not over the file's parsed `tools:` line — a hand-edited or corrupted generated agent file passes `-VerifyOnly` as long as the config and catalog it was derived from still would produce a safe grant. | Deliberate scope boundary for this slice, per section 8's existing sentence: "If A3 passes and the file still grants a writer, the generator is wrong, not the gate." Closing it means parsing the file's `tools:` line and asserting set-equality against the derived grant as a new check (a future `A5`) — filed as follow-up work, not done here. |
