...
| **ghidrasql** | `0xeb/ghidrasql` v0.0.6 | SSE, **:8771**, native binary + logon Scheduled Task | ✅ | SQL query layer over a Ghidra project via the LibGhidraHost extension. Connection mode wired (`--ghidra`, `--project-name`, `--program`); the project is bootstrapped once with `ntdll.dll` imported+analyzed on first install (a `.bootstrapped` marker, written only after a successful import, makes re-installs a no-op). Live-verified: `ghidrasql_query` answers a real `SELECT COUNT(*) FROM funcs`. |
...
- ~~**Skills and agents**~~ — **both shipped since, as follow-on slices.** Eight skill packs are vendored and signed off, and the three-agent topology is built and gated. `dariushoule/x64dbg-skills` alone is still deferred. See `HANDOFF.md`.
- **Hooks** — VM snapshot before dynamic tools, output redaction, untrusted-data tagging, write approval — 🔻 **low priority as of 2026-09-09**
- **Audit trail** (C2) — 🔻 **low priority as of 2026-09-09**. The **verification oracle** (C6 — evaluate `2akouwu/reverify` before building) is *not* deprioritised: it is capability, and stays a live candidate
- **Cost caps** (C4) and **kill switch / resource containment** (C5) — 🔻 **low priority as of 2026-09-09**
...
| 2026-09-07 | **Whole-branch review of the skills-vendoring subsystem, and its fixes.** Five real defects found by reading the code against the spec and reproducing each one. (1) A pack with `enabled: false` called `Remove-OrphanedSkill -Wanted @()` against the **shared** `.claude\skills` root, deleting every other pack's skills — and because each run reinstalls then re-deletes them, never converging; removal is now scoped to the pack's own names. (2) `$wrote = $wrote -or (Write-SkillPackFile ...)` short-circuits, so only the **first** enabled skill in a pack was ever written — `windbg` and `re` each ship two. (3) `Get-SkillCheck` skipped the adaptation gate for any pack that was not installed, which with every pack held at the review gate meant G0–G2/G4 ran **nowhere**, contradicting spec §10.2; adaptation and drift are now gated independently. (4) G2 read only `SKILL.md`, so an upstream tool name surviving in one of the dotnet pack's fifteen reference files passed the gate. (5) `Get-RecordedSkillResult` threw under StrictMode on a config with no `skills` key, taking phase 6 down for any pre-skills config. Also: the two disabled windbg skills drove five MCP tools while declaring none, so enabling them would have shipped no runtime grant and a vacuous G1 pass; and `CLAUDE.md.template` gained its `## Skills` contract section (spec §9) and lost the same false "no live-process tool" claim the tool catalog was already corrected for. 430 tests, zero analyzer findings. |
| 2026-09-07 | **`allowed-tools` enforcement measured, and it does not enforce.** Task 0 established only that Claude Code *accepts* the key; whether it *restricts* a skill was left explicitly unverified, and four places had since promoted it to "a real permission grant, not documentation" - including the sign-off checklist that gates all five packs. Measured with an A/B probe on 2.1.263: two `.claude/skills/zz-toolgate/SKILL.md` files identical but for the header. Declaring only `Read`, the skill ran `Bash` anyway, both with and without a session grant; declaring `Read, Bash`, it was still denied `Bash` under `--disallowedTools Bash`, reporting the tool absent from its set entirely. So the key neither grants nor restricts - session configuration decides - and the debug logs never echo it, the same blind spot Task 0 hit. All four claim sites corrected to call it a declaration of intent. G1 is unaffected, being a static comparison against the pinned catalog. |
| 2026-09-08 | **Skills vendoring complete.** Eight packs vendored, adapted and **signed off by hand** (`reviewedBy: david`), and installed: `windbg`, `ghidra`, `re`, `route`, `reva`, `dotnet`, `tob`, `arch` — eleven skill directories under `C:\re\agent\.claude\skills`. Task 19 (x64dbg) stays deferred by decision. Task 22 (local marketplace manifest) was **measured, not built**: a local marketplace is not auto-discovered, one-plugin-per-pack cannot be expressed over the locked flat layout, and Claude Code keeps flat and plugin skill names live at once, so the toggle would not toggle anything off. Task 20 (Binary Ninja) is the one pack still owed, blocked on an attended `-UpdateToolCatalog` capture. |
| 2026-09-09 | **Agent topology complete, and the MVP is marked done.** Three subagents — `static-analyst` (write), `verifier` (read), `dynamic-analyst` (write, shipped disabled) — are declared in `re-agent.config.json`, generated into `.claude\agents\` by phase 4 from templates, and gated A0–A4 in phase 6. Every grant is **derived** from `data/tool-catalog.json`'s new `classification` block rather than hand-listed; `destructive` is granted to nobody at any level. A review round then fixed four real defects: the forbidden-builtin **denylist** was bypassable by a parameterized form like `Bash(python:*)` and became an allowlist of `Read`/`Glob`/`Grep`; `Write-Manifest` hardcoded `gate: 'pass'` for every agent, so a run whose gate failed still wrote a manifest claiming it passed; the routing contract named `dynamic-analyst` unconditionally though it ships disabled; and spec §12 gained gap 9, that the gate validates the derived grant and never the generated file's bytes. **535 tests, zero analyzer findings.** All five MVP definition-of-done items remain met; the one open item across the whole repo is the Binary Ninja skill pack. |
