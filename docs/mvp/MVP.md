...
| **ghidrasql** | `0xeb/ghidrasql` v0.0.6 | SSE, **:8771**, native binary + logon Scheduled Task | ⛔ installed, disabled | SQL query layer over a Ghidra project via the LibGhidraHost extension. ⚠️ Ships disabled: its launcher has no `--ghidra <path>` connection-mode flag wired yet, so the binary cannot start. Declared and classified read-only (Q0-gated) for when that wiring lands. |
